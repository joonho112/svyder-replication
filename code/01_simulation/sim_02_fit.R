...
#' @param gate_config List or NULL. Convergence-gate configuration (see
#'   run_convergence_gate()). NULL resolves to SIM_PARAMS$convergence_gate
#'   if available, else the strict defaults.
#'
#' @return A list with components:
#'   fit: CmdStanMCMC object (or NULL if fitting failed),
#'   convergence: structured gate verdict from run_convergence_gate()
#'     (or NULL),
#'   timing: wall-clock seconds,
#'   error: error message or NULL,
#'   succeeded: logical.
fit_hlr <- function(data_list, stan_model, mcmc_settings = NULL,
                    seed = NULL, quiet = TRUE, gate_config = NULL) {
  ...
  # -- Check convergence ------------------------------------------------------
  # The gate is configurable (SIM_PARAMS$convergence_gate or the
  # gate_config argument): "strict" (default; Rhat <= 1.01 on ALL model
  # parameters, zero divergences) or "v1_weak" (a weaker alternative gate
  # on beta and sigma_theta only). See run_convergence_gate().
  convergence <- NULL
  if (!is.null(fit)) {
    convergence <- tryCatch(
      run_convergence_gate(fit, gate_config),
      error = function(e) {
        list(
          passed        = FALSE,
          verdict       = "error",
...
#
#   (a) check_convergence() -- a weaker alternative gate. It evaluates
#       Rhat <= 1.10 and ESS >= 100 on beta and sigma_theta ONLY (the J
#       individual theta_j are never monitored) and tolerates up to 5%
#       divergent transitions. It is retained as a documented alternative;
#       its thresholds are recorded in
#       SIM_PARAMS$convergence_gate$v1_weak.
#
#   (b) gate() / run_convergence_gate() -- the STRICT GATE (default).
#       It evaluates Rhat <= 1.01 and ESS >= 100 on ALL model parameters
#       (beta, sigma_theta, every theta_j, and the NCP eta_raw_j) and
#       requires ZERO divergent transitions, matching the convergence
#       standard claimed in the manuscript. It returns a structured
#       verdict with per-block diagnostics (rhat max by parameter block,
#       divergence counts) that run_single_rep() persists into every
#       per-replication output file, making convergence claims auditable.
#
# The active gate is selected by SIM_PARAMS$convergence_gate$version
# ("strict" by default, "v1_weak" for the weaker gate).
#
# Replications that fail the active gate are excluded from coverage
# analysis to ensure valid inference.
# =============================================================================

#' Check MCMC convergence diagnostics (legacy weak gate)
#'
#' This is a weaker alternative gate: it checks beta and sigma_theta only,
#' with permissive thresholds. Prefer the strict gate
#' (run_convergence_gate() with version = "strict") for new runs.
#'
...
#' Strict convergence gate on a fit summary (default)
#'
#' Evaluates Rhat / ESS thresholds over ALL parameters in the supplied
#' summary (including every theta_j) and an absolute divergence budget.
#' Pure function of its inputs; use run_convergence_gate() to drive it
#' from a CmdStanMCMC fit object.
#'
#' @param fit_summary Data frame from fit$summary() with columns
#'   variable, rhat, ess_bulk, ess_tail.
#' @param rhat_max Numeric. Maximum acceptable Rhat (default 1.01).
#' @param params "all" (default) to check every row of fit_summary, or a
#'   character vector of parameter base names (e.g., c("beta",
#'   "sigma_theta")) to restrict the check. Rows for lp__ and generated
#'   quantities (score_residual) are always excluded.
#' @param max_divergences Integer. Maximum acceptable number of divergent
#'   transitions (default 0).
#' @param min_ess Numeric. Minimum acceptable bulk/tail ESS (default 100).
#' @param n_divergent Integer. Observed number of divergent transitions
#'   (supplied by the caller; NA is treated as a failure).
#' @param total_iter Integer. Total post-warmup iterations (for the
#'   divergence percentage; may be NA).
#' @param n_max_treedepth Integer. Observed max-treedepth hits (recorded,
#'   not gated).
#'
#' @return A structured verdict list with fields: passed, verdict
#'   ("pass"/"fail"), gate (the thresholds applied), n_params_checked,
#'   params_checked, rhat_max, rhat_max_by_block, ess_bulk_min,
#'   ess_bulk_min_by_block, ess_tail_min, ess_tail_min_by_block,
#'   n_divergent, pct_divergent, n_max_treedepth, failures, details.
#'   Field names shared with check_convergence() (rhat_max, ess_bulk_min,
#'   ess_tail_min, n_divergent, pct_divergent, n_max_treedepth, passed,
#'   details) keep downstream consumers working under either gate.
gate <- function(fit_summary, rhat_max = 1.01, params = "all",
                 max_divergences = 0L, min_ess = 100,
                 n_divergent = NA_integer_, total_iter = NA_integer_,
                 n_max_treedepth = NA_integer_) {

  stopifnot(
    is.data.frame(fit_summary),
    all(c("variable", "rhat", "ess_bulk", "ess_tail") %in%
          names(fit_summary))
  )

  vars <- fit_summary$variable

  ## Always exclude lp__ and generated quantities (deterministic
  ## per-draw transforms; score_residual is N-dimensional)
  keep <- !grepl("^lp__$|^score_residual(\\[|$)", vars)

  ## Optional restriction to named parameter blocks
  if (!identical(params, "all")) {
    base_all <- sub("\\[.*$", "", vars)
    keep <- keep & (base_all %in% params)
  }

  summ <- fit_summary[keep, , drop = FALSE]
  if (nrow(summ) == 0L) {
    stop("gate(): no parameters left to check after filtering.")
  }

  ## Per-block diagnostics (block = parameter base name, e.g. "theta")
  base_name <- sub("\\[.*$", "", summ$variable)
  rhat_max_by_block     <- c(tapply(summ$rhat,     base_name, max, na.rm = TRUE))
  ess_bulk_min_by_block <- c(tapply(summ$ess_bulk, base_name, min, na.rm = TRUE))
  ess_tail_min_by_block <- c(tapply(summ$ess_tail, base_name, min, na.rm = TRUE))

  rhat_max_obs <- suppressWarnings(max(summ$rhat,     na.rm = TRUE))
  essb_min_obs <- suppressWarnings(min(summ$ess_bulk, na.rm = TRUE))
  esst_min_obs <- suppressWarnings(min(summ$ess_tail, na.rm = TRUE))

  pct_divergent <- if (!is.na(n_divergent) && !is.na(total_iter) &&
                       total_iter > 0) {
    n_divergent / total_iter
  } else {
    NA_real_
  }
  ...
  rhat_ok <- is.finite(rhat_max_obs) && rhat_max_obs <= rhat_max
  if (!rhat_ok) {
    failures <- c(failures,
                  sprintf("Rhat max = %.4f (threshold: %.3f)",
                          rhat_max_obs, rhat_max))
  }

  ess_bulk_ok <- is.finite(essb_min_obs) && essb_min_obs >= min_ess
  if (!ess_bulk_ok) {
    failures <- c(failures,
                  sprintf("ESS bulk min = %.0f (threshold: %.0f)",
                          essb_min_obs, min_ess))
  }

  ess_tail_ok <- is.finite(esst_min_obs) && esst_min_obs >= min_ess
  if (!ess_tail_ok) {
    failures <- c(failures,
                  sprintf("ESS tail min = %.0f (threshold: %.0f)",
                          esst_min_obs, min_ess))
  }

  div_ok <- !is.na(n_divergent) && n_divergent <= max_divergences
  if (!div_ok) {
    failures <- c(failures,
                  if (is.na(n_divergent)) {
                    "Divergence count unavailable"
                  } else {
                    sprintf("Divergent transitions: %d (budget: %d)",
                            n_divergent, max_divergences)
                  })
  }

  passed <- rhat_ok && ess_bulk_ok && ess_tail_ok && div_ok
  details <- if (passed) {
    sprintf("Strict gate passed on %d parameters (%s).",
            nrow(summ), paste(sort(unique(base_name)), collapse = ", "))
  } else {
    paste("STRICT GATE FAILURES:", paste(failures, collapse = "; "))
  }

  list(
    passed                = passed,
    verdict               = if (passed) "pass" else "fail",
    gate                  = list(rhat_max        = rhat_max,
                                 params          = params,
                                 max_divergences = max_divergences,
                                 min_ess         = min_ess),
    n_params_checked      = nrow(summ),
    params_checked        = sort(unique(base_name)),
    rhat_max              = rhat_max_obs,
    rhat_max_by_block     = rhat_max_by_block,
    ess_bulk_min          = essb_min_obs,
    ess_bulk_min_by_block = ess_bulk_min_by_block,
    ess_tail_min          = esst_min_obs,
    ess_tail_min_by_block = ess_tail_min_by_block,
    n_divergent           = n_divergent,
    pct_divergent         = pct_divergent,
    n_max_treedepth       = n_max_treedepth,
    failures              = failures,
...
#' Run the configured convergence gate on a CmdStanMCMC fit
#'
#' Dispatches between the strict gate (gate(), default) and the legacy weak
#' gate (check_convergence()) according to the configuration.
#'
#' @param fit CmdStanMCMC object.
#' @param gate_config List or NULL. NULL resolves to
#'   SIM_PARAMS$convergence_gate if available, else the strict defaults.
#'   Structure: list(version = "strict"|"v1_weak", strict = list(...),
#'   v1_weak = list(...)); see sim_00_config.R.
#'
#' @return A structured verdict list (see gate()); the field gate_version
#'   records which gate was applied.
run_convergence_gate <- function(fit, gate_config = NULL) {

  if (!inherits(fit, "CmdStanMCMC")) {
    stop("fit must be a CmdStanMCMC object.")
  }

  if (is.null(gate_config)) {
    gate_config <- if (exists("SIM_PARAMS") &&
                       !is.null(SIM_PARAMS$convergence_gate)) {
      SIM_PARAMS$convergence_gate
    } else {
      list(version = "strict")
    }
  }
  ...
  if (is.null(version)) version <- "strict"

  ## -- Legacy weak gate -------------------------------------------------------
  if (identical(version, "v1_weak")) {
    cfg <- gate_config$v1_weak
    if (is.null(cfg)) cfg <- list()
    res <- check_convergence(
      fit,
      rhat_threshold = if (is.null(cfg$rhat_max))    1.10 else cfg$rhat_max,
      min_ess        = if (is.null(cfg$min_ess))     100  else cfg$min_ess,
      max_div_pct    = if (is.null(cfg$max_div_pct)) 0.05 else cfg$max_div_pct
    )
    res$verdict      <- if (res$passed) "pass" else "fail"
    res$gate_version <- "v1_weak"
    return(res)
  }

  ## -- Strict gate (default) ---------------------------------------------------
  cfg <- gate_config$strict
  if (is.null(cfg)) cfg <- list()
  params          <- if (is.null(cfg$params))          "all" else cfg$params
  rhat_max        <- if (is.null(cfg$rhat_max))        1.01  else cfg$rhat_max
  max_divergences <- if (is.null(cfg$max_divergences)) 0L    else cfg$max_divergences
  min_ess         <- if (is.null(cfg$min_ess))         100   else cfg$min_ess

  ## "all" = all sampled/transformed model parameters of hlr_weighted.stan
  ## (generated quantities are excluded; they are deterministic transforms)
  vars <- if (identical(params, "all")) {
    c("beta", "sigma_theta", "theta", "eta_raw")
  } else {
    params
  }
